...
## -- Frequency Encoding: int_rate, sub_grade, emp_title, emp_length, issue_d, url, desc, purpose, title, zip_code, addr_state, earliest_cr_line, revol_util_freq, last_pymnt_d_freq, last_credit_pull_d_freq    -- 
...
recipe <- recipe(loan_status ~ ., data = train) %>% 
  ...
  step_rm(member_id, next_pymnt_d, mths_since_last_delinq, mths_since_last_record, application_type) %>% 
...
xgb_model <- boost_tree(trees = 115,
                        tree_depth = 15,
                        min_n = 7,
                        learn_rate = .005) %>%
  set_engine("xgboost") %>%
...
predict(xgb_workflow, train) %>%
  bind_cols(predict(xgb_workflow, train, type = "prob"))%>%
  bind_cols(train) -> train_scored_xgb

predict(xgb_workflow, test) %>%
  bind_cols(predict(xgb_workflow, test, type = "prob")) %>%
...
train_scored_xgb <- model_score(train, xgb_workflow, "xgb training")
test_scored_xgb <- model_score(test, xgb_workflow, "xgb testing")
...
predict(rf_final_fit, holdout, type = "prob") %>%
  ...
  bind_cols(., holdout) -> holdout_score_rf

holdout_rf <- holdout_score_rf %>%
  mutate(loan_status = as.factor(.pred_class)) %>%
  dplyr::select(id, loan_status)

write_csv(holdout_rf, "Project_6_RF_Preds2_TylerGrosman.csv")
```

```{r, message = FALSE, warning = FALSE}
predict(xgb_final_fit, holdout, type = "prob") %>%
  bind_cols(predict(xgb_final_fit, holdout, type = "class")) %>%
  bind_cols(., holdout) -> holdout_score_xgb

holdout_xgb <- holdout_score_xgb %>%
  mutate(loan_status = as.factor(.pred_class)) %>%
  dplyr::select(id, loan_status)

write_csv(holdout_xgb, "Project_6_XGB_Preds2_TylerGrosman.csv")
```
