---
title: "Project 2: BAN 6053 - Introduction to Machine Learning"
...
fraud_holdout <- read_csv("project_2_holdout.csv") %>%
  clean_names() %>%
  mutate(user_agent = str_sub(user_agent, 1, 11)) %>%
  mutate(event_timestamp = as.Date.character(event_timestamp, format = "%Y-%m-%d")) %>%
  mutate(billing_postal = round(as.numeric(billing_postal))) %>%
...
kaggle <- read_csv("project_2_holdout.csv") %>% subset(select = -score)
```
...
fraud_original %>%
  mutate(event_label = as.character(churn)) %>%
  ggplot(aes(....., fill = churn)) +
...
kaggle <- holdout_score %>%
  mutate(event_label = as.factor(if_else(.pred_fraud >= 0.06, 1, 0))) %>%
  subset(select = c("event_id", "event_label"))

kaggle

write_csv(kaggle, "Project_2_Preds_Tyler_Grosman.csv")

precision(holdout_score, event_label, .pred_class)
recall(holdout_score, EVENT_LABEL, .pred_class)
